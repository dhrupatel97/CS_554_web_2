...
const ComicList = (props) => {
  ...
  return (
    <div className='main'>
      {comData && pageNumber > comData.count / pageSize || pageNumber < 0 ? (<p className='error'>Error</p>) : (
        <div>
          <div className='pagination'>
          {comData && paramValue != 0 ?  (<a href={`/comics/page/${paramValue - 1}`}><button className='btn'>Previous</button></a>) : (<p></p>)}
          {comData && paramValue != 4 ?(<a href={`/comics/page/${paramValue + 1}`}><button className='btn'>Next</button></a>) :  (<p></p>)}
      </div>
        <div className='cards'>
          {comData && comData.results.slice(paramValue * pageSize, (paramValue + 1) * pageSize).map((item) => {
            return (
              <div class="card" key={item.id}>
                <a>
                  <img className='card-img' src={getImage(item.thumbnail)} alt={item.title}/>
                  <div className='card-body'>
                  <Link to={`/comics/${item.id}`}><h2>{item.title}</h2></Link>
